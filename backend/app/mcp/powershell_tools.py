...
def get_powershell_processes():
    ...
    for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
        try:
            if proc.info['name'] and 'powershell' in proc.info['name']:
                processes.append({
                    'pid': proc.info['pid'],
                    'name': proc.info['name'],
...
def activate_powershell_window():
    ...
    try:
        ...
        windows = pyautogui.getWindowsWithTitle('Windows PowerS')
        if not windows:
...
def open_new_powershell(working_directory: Annotated[str, Field(description="optional working directory,empty then use current directory")] = "") -> str:
    ...
    try:
        ...
        if returncode != 0 and stderr:
            return f"open PowerShell failed: {stderr}"
        
        time.sleep(2)
        processes = get_powershell_processes()
        return f"PowerShell has been opened,current running process count: {len(processes)}"
    except Exception as e:
        return f"open PowerShell failed: {str(e)}"
...
@mcp.tool(name="run_powershell_script", description="send command to active PowerShell window by pyautogui")
def run_powershell_script(script: Annotated[str, Field(description="command to be executed in PowerShell window")]) -> str:
    """send command to active PowerShell window by pyautogui"""
    try:
        print("-" * 50)
